 ...
 * @param Credits Text
 * @parent Window Options
 * @desc Text to display at the top of the credits window and in title command window.
 * @default Credits
*/
...
CGMZ_Temp.prototype.registerPluginCommands = function() {
	...
	PluginManager.registerCommand("CGMZ_Credits", "callScene", this.pluginCommandCreditsCallScene);
};
//-----------------------------------------------------------------------------
...
CGMZ_Window_Credits.prototype.drawCredits = function(y) {
	...
	for(let i = 0; i < credits.length; i++) {
		...
		for(let j = 0; j < creditEntries.length; j++) {
			this.CGMZ_drawText(creditEntries[j].Name, 0, 0, y, this.contents.width, 'center');
			y += this.lineHeight();
			if(creditEntries[j].Comment) {
				this.CGMZ_drawText(creditEntries[j].Comment, 0, 0, y, this.contents.width, 'center');
				y += this.lineHeight();
			}
			if(creditEntries[j].URL) {
				this.CGMZ_drawText(creditEntries[j].URLText, 0, 0, y, this.contents.width, 'center');
				let urlObject = {};
				urlObject.url = creditEntries[j].URL;
				urlObject.rect = new Rectangle(0, y, this.contents.width, this.lineHeight());
...
// Scene_Title
//-----------------------------------------------------------------------------
// Show credits in title screen
//=============================================================================
//-----------------------------------------------------------------------------
// Add credits command to title window
//-----------------------------------------------------------------------------
const alias_CGMZ_Credits_createCommandWindow = Scene_Title.prototype.createCommandWindow;
Scene_Title.prototype.createCommandWindow = function() {
	alias_CGMZ_Credits_createCommandWindow.call(this);
    this._commandWindow.setHandler('credits',  this.CGMZ_commandCredits.bind(this));
};
//-----------------------------------------------------------------------------
// Handling for credits command
//-----------------------------------------------------------------------------
Scene_Title.prototype.CGMZ_commandCredits = function() {
...
// Add credits command to title window
//-----------------------------------------------------------------------------
const alias_CGMZ_Credits_makeCommandList = Window_TitleCommand.prototype.makeCommandList;
Window_TitleCommand.prototype.makeCommandList = function() {
	alias_CGMZ_Credits_makeCommandList.call(this);
    this.addCommand(CGMZ.Credits.WindowTitle, 'credits');
};