 ...
 * @type note[]
 * @default []
 * @desc The main description of the quest
 *
 * @param Objectives
 ...
 * @type boolean
 * @default false
 * @desc Enable this to automatically track the progress if item, weapon, armor, or gold
 *
 * @param Gold Tracking
 * @type number
 * @default 0
...
CGMZ.Versions["Quest System"] = "Alpha";
CGMZ.QuestSystem = CGMZ.QuestSystem || {};
...
CGMZ_Temp.prototype.initializeQuestData = function() {
	...
	this._questData = {};
	this._questBoardSettings = {questList: [], img: ""};
...
CGMZ_Temp.prototype.checkAutomaticGoldQuests = function() {
	for(let i = this._autoTrackQuests.gold.length - 1; i >= 0; i--) {
		...
		for(const objective of objectives) {
			if(objective.goldTracking > 0 && gold >= objective.maxProgress) {
...
CGMZ_Temp.prototype.checkAutomaticVariableQuests = function() {
	for(let i = this._autoTrackQuests.variable.length - 1; i >= 0; i--) {
		...
		for(const objective of objectives) {
			...
			if(objective.variableTracking > 0 && value >= objective.maxProgress) {
				questState.setObjectiveComplete(objective.id, true);
				this.checkForQuestStageAdvance(quest, objectives);
			}
		}
...
CGMZ_Temp.prototype.pluginCommandQuestSystemCallQuestBoardScene = function(args) {
	const questBoardObject = {questList: JSON.parse(args["Quest Names"]), img: args["Scene Image"]};
...
CGMZ_Temp.prototype.pluginCommandQuestSystemSetObjectiveProgress = function(args) {
	...
	const questObjective = $cgmzTemp.getQuest(args["Quest Name"]).getObjectiveById(args.id);
	if(quest && questObjective) {
		let objectiveProgress = quest.getObjectiveProgress(args.id);
		switch(args.mode) {
...
CGMZ_Scene_QuestSystem.prototype.listWindowRect = function() {
	...
	const width = Graphics.boxWidth * 0.40;
...
CGMZ_Window_QuestList.prototype.update = function() {
	...
	if($cgmzTemp.isKeyPressed(CGMZ.QuestSystem.SortKey)) {
...
CGMZ_Window_QuestList.prototype.drawItem = function(index) {
	...
	if(item.isCategory) {
		...
		const string = prefix + this._category._displayName + " (" + item.amount + ")";
		this.contents.fontSize = $gameSystem.mainFontSize();
		this.changeTextColor(ColorManager.textColor(this._category._textColor));
		this.drawText(string, rect.x, rect.y, rect.width, 'left');
	...
	//	this.contents.fontSize -= 4;
		this.drawText(item.questName, rect.x + xOffset, rect.y, rect.width, 'left');
		this.drawText(quest._location, rect.x + xOffset, rect.y + this.lineHeight(), rect.width, 'left');
...
CGMZ_Window_QuestDisplay.prototype.drawQuestInfo = function(quest, questState, showImage) {
	...
	for(const infoType of CGMZ.QuestSystem.QuestInfoOrder) {
		switch(infoType) {
			case "Divider - Info":
				this.CGMZ_drawHeader(CGMZ.QuestSystem.InfoText, y);
				y += this.lineHeight(); break;
			case "Divider - Description":
				this.CGMZ_drawHeader(CGMZ.QuestSystem.DescriptionText, y);
				y += this.lineHeight(); break;
			case "Divider - Objectives":
				this.CGMZ_drawHeader(CGMZ.QuestSystem.ObjectivesText, y);
				y += this.lineHeight(); break;
			case "Divider - Rewards":
				this.CGMZ_drawHeader(CGMZ.QuestSystem.RewardsText, y);
				y += this.lineHeight(); break;
			...
			case "Rewards": y += this.drawRewards(quest, y);
		}
	}
...
CGMZ_Scene_QuestSystemAcceptQuest.prototype.onAccept = function() {
	const quest = $cgmz.getQuest(this._quest);
	quest.start(true, true);
...
CGMZ_Scene_QuestSystemQuestBoard.prototype.initialize = function() {
    Scene_MenuBase.prototype.initialize.call(this);
	this._questList = $cgmzTemp._questBoardSettings.questList;
	this._sceneImage = $cgmzTemp._questBoardSettings.img;
};
//-----------------------------------------------------------------------------
// Create scene windows
//-----------------------------------------------------------------------------
...
CGMZ_Scene_QuestSystemQuestBoard.prototype.createQuestBoardWindow = function() {
    ...
    this._questBoardWindow = new CGMZ_Window_QuestBoardSelectable(rect, this._questList);
...
CGMZ_Scene_QuestSystemQuestBoard.prototype.questBoardWindowRect = function() {
	const width = Graphics.boxWidth * 0.8;
	const height = Graphics.boxWidth * 0.6;
	const x = Graphics.boxWidth / 2 - width / 2;
	const y = Graphics.boxHeight / 2 - height / 2;
	return new Rectangle(x, y, width, height);
};
...
// Initialize
//-----------------------------------------------------------------------------
CGMZ_Window_QuestBoardSelectable.prototype.initialize = function(rect, questList) {
...
CGMZ_Window_QuestBoardSelectable.prototype.makeItemList = function() {
	this._data = this._questList.filter(questName => $cgmz.getQuest(questName) && !$cgmz.getQuest(questName)._isStarted);
};
//-----------------------------------------------------------------------------
